...
from gehenna_api.schemas import Message, SlotList, SlotPublic, SlotQuantitySchema, SlotSchema
...
@router.get('/{id}', response_model=SlotList)
def read_slots(
        id: int, 
        skip: int = 0, 
        limit = 100, session: Session = Depends(get_session)):
    slots = session.scalars(select(Slot).offset(skip).limit(limit)).all()
    return {'slots': slots}
