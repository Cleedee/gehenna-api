...
class DeckService(BaseService):
    ...
    def get_decks(self, owner_id: int, skip: int = 0, limit: int = 100) -> List[DeckPublic]:
        return DeckDataManager(self.session).get_decks(owner_id, skip, limit)

    def get_decks_by_user(self, username, skip: int = 0, limit: int = 100) -> List[DeckPublic]:
        return DeckDataManager(self.session).get_decks_by_user(username, skip, limit)
...
class DeckDataManager(BaseDataManager):
    ...
    def get_decks(self, owner_id: int, skip: int = 0, limit: int = 100) -> List[DeckPublic]:
        ...
        stmt = select(Deck).where(Deck.owner_id == owner_id).offset(skip).limit(limit)
        for model in self.get_all(stmt):
            schemas += [DeckPublic(**model.to_dict())]
        return schemas

    def get_decks_by_user(self, username: str, skip: int = 0, limit: int = 100) -> List[DeckPublic]:
        # TODO 
        schemas: List[DeckPublic] = list()
